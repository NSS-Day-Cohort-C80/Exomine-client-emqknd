...
export const purchaseMineral = async () => {
  ...
  const colonyMineralResponse = await fetch(
    `http://localhost:5091/api/colonyMinerals?colonyId=${governor.colonyId}&mineralId=${transientState.mineralId}`,
  );
  ...
  if (colonyMineralsArray.length === 0) {
    ...
    await fetch("http://localhost:5091/api/colonyMinerals", postOptions);
  } else {
    ...
    await fetch(
      `http://localhost:5091/api/colonyMinerals/${colonyMineral.id}`,
      putColonyOptions,
    );
  }
  ...
  const facilityMineralResponse = await fetch(
    `http://localhost:5091/api/facilityMinerals?facilityId=${transientState.facilityId}&mineralId=${transientState.mineralId}`,
  );
  ...
  await fetch(
    `http://localhost:5091/api/facilityMinerals/${facilityMineral.id}`,
    putFacilityOptions,
  );
